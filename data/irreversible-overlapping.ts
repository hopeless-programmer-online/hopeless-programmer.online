...
const note_1 = note(sentence(`У цій статті я буду вживати термін `, fig(`перекриття`), ` замість звичного `, fig(`приховування`), `. `));
const note_2 = note(sentence(`Саме тому я називаю це перекриття `, fig(`незворотним`), `. `));
