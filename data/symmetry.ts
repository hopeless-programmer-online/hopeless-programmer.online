import { illustration, paragraph, sentence, link, section, article, note, list, ref, fig, neg, kw, cm, cpp, c, f, v, lt, js, emp, code, cs, lua, py } from "../classes/shortcuts";
...
const code_void = illustration(`В С++ неможливо записати функцію без вказування типу значення яке вона повертає. `, cpp(...[
...
const code_out_cs = illustration(sentence(cs(kw(`out`)), ` аргументи в C#. `), cs(...[
...
const code_return_js = illustration(`Різниця між поверненням багатьох результатів та поверненням масиву в JavaScript. `, js(...[
...
const code_out_cpp = illustration(`Використання посилань в С++ для отримання додаткових результатів. `, cpp(...[
...
const code_return_py = illustration(`Повернення багатьох значень з функції в Python. `, py(...[
    [ kw(`def`), ` `, f(`f`), `():` ],
    [ `    `, kw(`return`), ` `, lt(`1`), `, `, lt(`2`), `, `, lt(`3`), `` ],
    [ `` ],
    [ v(`x`), `, `, v(`y`), `, `, v(`z`), ` = `, f(`f`), `()` ],
...
const code_return_tuple_py = illustration(sentence(`Отримання багатьох результатів як `, py(c(`tuple`)), `. `), py(...[
    [ kw(`def`), ` `, f(`f`), `():` ],
    [ `    `, kw(`return`), ` `, lt(`1`), `, `, lt(`2`), `, `, lt(`3`), `` ],
    [ `` ],
    [ v(`x`), ` = `, f(`f`), `()` ],
...
const code_return_lua = illustration(`Повернення багатьох значень з функції в Lua. `, lua(...[
    [ kw(`function`), ` `, f(`f`), `()` ],
    [ `    `, kw(`return`), ` `, lt(`1`), `, `, lt(`2`), `` ],
    [ kw(`end`) ],
    [ `` ],
...
export default article([ `Думки вголос. `, `Асиметрія аргументів та результатів функції. ` ], {}, ...[
    section(`Асиметрія.`, ...[
        paragraph(...[
            ...
            sentence(`Я не знайшов окремого терміну для позначення цього явища, тому надалі буду називати його просто `, fig(`асиметрією`), `. `),
            sentence(`Так от, `, fig(`асиметрія`), ` - це дуже тупо. `),
    ...
    /**
     * Якось до Конфуція в гості завітала дуже освічена пані і спитала:
    ...
    section(`Про що взагалі мова?`, ...[
        paragraph(...[
            ...
            sentence(`А от з результатом так не вийде: ви повинні або вказати його тип, або вказати `, void_, ` (див. `, code_void, `)`, note_void_res, `. `),
        ]),
        code_void,
        paragraph(...[
            ...
            sentence(`В деяких з них (наприклад, в С#) вони навіть стають частиною мови і перетворюються на окремі службові слова (див. `, code_out_cs, `). `),
        ]),
        code_out_cs,
        paragraph(...[
    ...
    section(`Повертайте структуру! `, ...[
        paragraph(...[
            ...
            sentence(`В мовах без статичної типізації, таких як JavaScript, з цього взагалі не роблять проблему, адже навіть різниця в записі нікчемна (див. `, code_return_js, `). `),
        ]),
        code_return_js,
        paragraph(...[
        ...
        paragraph(...[
            ...
            sentence(`Ну і на останок: це `, emp(`не`), ` є закликом завжди повертати набір об'єктів замість структури і масиву. `),
    ...
    section(`Output-аргументи. `, ...[
        paragraph(...[
            sentence(`Друга стратегія - це передача в функцію посилань на об'єкти, які виступатимуть `, fig(`додатковими`), ` результатами. `),
            ...
            sentence(`Класичним прикладом може виступати функція для створення об'єкту, результат якої говорить про статус виконання, а output-аргумент виступає посиланням на сам об'єкт (див. `, code_out_cpp, `). `),
        ]),
        code_out_cpp,
        paragraph(...[
    ...
    section(`Популярні мови повертають лише одне значення. `, ...[
        ...
        paragraph(...[
            sentence(`В Python функція може повернути декілька значень, якщо записати їх через кому (див. `, code_return_py, `). `),
            ...
            sentence(`Якщо ж в місці виклику вказана лише одна змінна, то значення не будуть розпаковані а сама змінна буде вказувати на `, tuple_py, ` (див. `, code_return_tuple_py, `). `),
        ]),
        code_return_py,
        ...
        paragraph(...[
            ...
            sentence(`Зокрема, на відміну він Python, якщо значень бракуватиме їх місця будуть заповнені `, nil, ` (див. `, code_return_lua, `). `),
    ...
    section(`Приклади функцій з кількома результатами. `, ...[
        paragraph(...[
            ...
            sentence(`А ще є ціла низка функцій (і навіть мов), які замість виключень додатково повертають `, link(`коди помилок`, c_errors), `. `),
    ...
    section(`Підбиваємо підсумки. `, ...[
        paragraph(...[
            sentence(`Отож, аргументи `, emp(`проти`), ` повернення кількох результатів щонайменше будуть цікаві авторам Python та Lua. `),
