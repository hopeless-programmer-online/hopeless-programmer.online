...
export default article(`Знаходження точки перетину рухомої сфери та прямої. `, ...[
    ...
    section('', ...[
        paragraph(...[
            sentence('Ми можемо розрахувати точку дотику сфери до необмеженої прямої, але не до обмеженої лінії. '),
            sentence('Для того аби сфера могла проходити крізь пряму в місцях поза лінією нам потрібно визначити де саме лежить точка контакту цієї прямої зі сферою. '),
            sentence(' '),
            sentence(' '),
        ]),
        illustration(``, tex(r`
            \begin{cases}
                ...
                \vec{x}' &= \vec{u} + t'\vec{w}
                \\
                \vec{x}' &= \vec{u} + \frac{ (\vec{x} - \vec{u}, \vec{w}) }{ \vec{w}^2 } \vec{w}
            \end{cases}
        `)),
        illustration(``, tex(r`
            \begin{cases}
                \vec{x} &= \vec{a} + t\vec{d}
                \\
                t' =& \frac{ (\vec{x} - \vec{u}, \vec{w}) }{ \vec{w}^2 }
            \end{cases}
        `)),
    ]),
])
