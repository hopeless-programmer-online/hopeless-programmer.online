...
export default  article([ sentence(`Мова програмування `, L0, `. `), sentence(`Програмований контроль типів. `) ], {}, ...[
    section(sentence(`Що таке `, L0, `? `), ...[
        paragraph(...[
            ...
            sentence(`Але це від неї і не вимагається, адже `, L0, ` створювалась винятково в якості прикладу `, link(`доведення концепції`, poc), ` програмованого контролю типів. `),
    ...
    section(sentence(`Мова програмування `, L0, `. `), ...[
        paragraph(...[
            ...
            sentence(`Також як і функції в багатьох мовах, програми в `, L0, ` є об'єктами `, link(`першого класу`, firstClass), `: їх можна передавати в якості аргументів та повертати. `),
        ]),
        illustration(``, l0(...[
            [ `; оголошення програми` ],
            [ `main() {` ],
        ...
        paragraph(...[
            sentence(`Основна відмінність програм від функцій полягає в механізмі повернення, який `, L0, ` реалізує через `, link(`передачу продовжень`, cps), `. `),
            sentence(`Безпосередньо перед викликом програми увесь наступний код загортається в `, link(`callback`, callback), `: програму-продовження, яка передається першим неявним аргументом `, l0(kw(`super`)), `. `),
            ...
            sentence(`Таким чином, повернення в `, L0, ` це лише `, link(`синтаксичний цукор`, sugar), ` навколо передачі продовження. `),
        ]),
        illustration(``, comparison(
            l0(...[
                [ `; оголошення програми` ],
        ...
        paragraph(...[
            sentence(`Під час свого оголошення програма `, link(`замикає`, closure), ` усі змінні оголошені до неї`, note(`враховуючи посилання на саму себе`), `, даючи можливість звернутись до них під час виклику. `),
        ...
        paragraph(...[
            ...
            sentence(`На основі цього працює як звичайне оголошення програми, так і оголошення продовження для виклику. `),
        ]),
        illustration(``, l0(...[
    ...
    section(`Аналіз структури програми. `, ...[
        paragraph(...[
            sentence(`Кожна програма в `, L0, ` по суті є першою ланкою ланцюжка інструкцій з яких вона складається. `),
            sentence(`Ми можемо звернутись до шаблону та сховища цієї інструкції за допомогою програм `, l0([ f(`get template`), `()` ]), ` та `, l0([ f(`get buffer`), `()` ]), `. `),
            sentence(``),
            sentence(``),
            sentence(``),
        ]),
