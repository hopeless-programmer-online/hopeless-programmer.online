...
import { article, c, cm, code, cs, emp, f, fig, illustration, js, jsx, kw, link, lt, neg, note, paragraph, py, quote, section, sentence, v, l0 } from '../classes/shortcuts'
...
const example_while = illustration('', l0(...[
...
const example_logic = illustration('', l0(...[
...
const example_if_return = illustration('', l0(...[
    [ '', kw('function'), ' ', f('main'), '() {' ],
    [ '    ', f('if'), ' (', lt('true'), ', () => {' ],
...
const example_if_index = illustration('', l0(...[
...
const example_server_client = illustration('', js(...[
    [ cm('// функція-обгортка для роботи в браузері') ],
    [ kw('function'), ' ', f('main'), '(', v('window'), ') {' ],
    [ '    ', v('window'), '.', f('addEventListener'), '(', lt('"load"'), ', () => {' ],
    [ '        ', v('console'), '.', f('log'), '(', lt('"window is loaded!"'), ');' ],
    [ '    });' ],
    [ '}' ],
    [ '' ],
    [ cm('// функція-обгортка для роботи на сервері') ],
    [ kw('function'), ' ', f('main'), '(', v('require'), ') {' ],
    [ '    ', kw('const'), ' ', v('fs'), ' = ', f('require'), '(', lt('"fs"'), ');' ],
    [ '    ' ],
    [ '    ', v('fs'), '.', f('writeSync'), '(', lt('"1.txt"'), ', ', lt('"server started!"'), ');' ],
    [ '}' ],
]))
const example_dependencies = illustration('', l0(...[
    [ kw('function'), ' ', f('main'), '(', v('if'), ', ', lt('true'), ', ', v('console'), ', ', lt('"this is true"'), ') {' ],
    [ '    ', f('if'), ' (', lt('true'), ', () => {' ],
    [ '        ', v('console'), '.', f('log'), '(', lt('"this is true"'), ');' ],
    [ '    });' ],
    [ '}' ],
]))
const example_overlapping_js = illustration('', js(...[
    [ kw('let'), ' ', v('x'), ' = ', lt('1'), ';' ],
    [ '' ],
    [ '{' ],
    [ '    ', cm('// доступ до x = 1 втрачений') ],
    [ '    ', kw('let'), ' ', v('x'), ' = ', lt('2'), ';' ],
    [ '}' ],
]))
const example_overlapping = illustration('', l0(...[
    [ kw('const'), ' ', v('x'), ' = ', lt('1'), ';' ],
    [ '' ],
    [ v('console'), '.', f('log'), '(', v('x'), '); ', cm('// 1') ],
    [ '' ],
    [ kw('const'), ' ', v('x'), ' = ', lt('2'), ';' ],
    [ '' ],
    [ v('console'), '.', f('log'), '(', v('x'), ');  ', cm('// 2') ],
    [ v('console'), '.', f('log'), '(/', v('x'), '); ', cm('// 1') ],
]))

const note_self = note(
    'Це такий жарт! ',
    sentence('Справа в тому, що мова програмування Python має одну особливість: методи класів в ній отримують доступ до власного об\'єкту через ', emp('явний'), ' аргумент ', py(kw('self')), '. '),
...
export default  article(sentence('Мова програмування ', L0, '. '), {}, ...[
    ...
    section(fig('Без назви. '), ...[
        paragraph(...[
            ...
            sentence('Сюди входять літерали на кшталт ', js(lt('true')), ', ', js(lt('123')), ' та ', js(lt('"ololo"')), ', стандартні арифметичні операції ', js('+'), ', ', js('-'), ' та решта, галуження, цикли, робота з масивами, стрічками та й взагалі звичними для нас контейнерами. '),
        ...
        paragraph(...[
            sentence('Аби пояснити що я маю на увазі і чого хочу досягти мені потрібно привести приклад. '),
            sentence('Одна з найпопулярніших сьогодні мов - JavaScript - відома тим, що на ній можна писати як код для веб сторінок що виконується в браузері, на стороні клієнта, так і код для серверів, які надсилають клієнтам ці сторінки. '),
            sentence('Одна мова - дві платформи. '),
            sentence('Найважливіше для нас з вами те, що це було б в принципі неможливо, якби мова JavaScript містила б в собі особливості намертво прив\'язані до якоїсь з платформ. '),
            sentence('Наприклад, ми можемо працювати з вікном браузера на стороні клієнта але не можемо працювати з файловою системою. '),
            sentence('З іншої сторони, ми можемо працювати з файловою системою на сервері, але не маємо доступу до вікна. '),
        ]),
        example_server_client,
        paragraph(...[
            sentence('Вікно та файли: ці дві сутності взаємовиключні для різних платформ, але мова від цього ніяк не змінюється. '),
            sentence('І все завдяки тому, що робота з ними відбувається не за рахунок ядра мови, а за рахунок, грубо кажучи, бібліотек. '),
            sentence('Вікно - ', js(v('window')), ' - звичайний об\'єкт, а функції для роботи з файлами - ', js([ v('fs'), '.', f('open') ]), ' та ', js([ v('fs'), '.', f('close') ]), ' - звичайні функції. '),
            sentence('Секрет успіху в тому, що в залежності від платформи - браузер або сервер - змінюються і глобальні змінні. '),
            sentence('Ми могли б вважати, що весь виконуваний код огортається функцією, яка приймає в якості аргументів глобальні змінні (див. ', example_server_client, '). '),
            sentence('І в залежності від середовища ці глобальні змінні наповнюються по різному. '),
        ]),
        paragraph(...[
            sentence(L0, ' має на меті розширити такий підхід до межі, абстрагувавшись не лише від невеликої кількості заздалегідь відомих об\'єктів а взагалі від всього. '),
            sentence('Якщо вважати, що JavaScript виносить в якості глобальних аргументів лише певний набір імен, то ', L0, ' робить глобальними аргументами взагалі всі неоголошені змінні. '),
            sentence('Сюди входять не лише імена які ми звикли надавати змінним чи функціям, але й імена усіх літералів: чисел та стрічок (див. ', example_dependencies, '). '),
            sentence('Абсолютно всі неоголошені об\'єкти перетворюється на залежності, які повинна (або не повинна) забезпечити платформа що виконує код. '),
        ]),
        example_dependencies,
        paragraph(...[
            sentence('Усі ', fig('незамінні'), ' фундаментальні будівельні елементи без яких не може бути написана програма ми отримуємо в якості глобальних аргументів. '),
            sentence('Але при цьому, незалежно від їх природи - галуження, цикли, арифметичні операції - усі вони надходять в нашу програму або як функції, або як об\'єкти які можна функціям згодувати. '),
            sentence('Таким чином ми можемо ввести в мову будь яку операцію, за умови що вона буде підтримуватись середовищем де виконується код. '),
        ]),
    ]),
    section('Боротьба з перекриттям. ', ...[
        paragraph(...[
            sentence('Тепер, коли проблема наповнення мови базовими об\'єктами та операціями вирішена, можна повернутись до незакритих питань. '),
            sentence('Одне з них може здатись несуттєвим, але воно має серйозний вплив на подальше просування. '),
            sentence('Друга ідея яку я вирішив випробувати в роботі над ', L0, ' - це оборотне перекриття імен. '),
            sentence('Справа в тому, що в більшості мов ми можемо перекривати (або маскувати, затінювати) вже оголошені об\'єкти. '),
            sentence('Якщо ми оголосимо де-небудь змінну ', js(v('x')), ', то після цього ми можемо оголосити змінну з таким же іменем у вкладеній області видимості. '),
            sentence('Всередині цієї області ми вже не зможемо звернутись до зовнішньої змінної, відтак в більшості мов доступ до неї буде втраченим (див. ', example_overlapping_js, '). '),
        ]),
        example_overlapping_js,
        example_overlapping,
        paragraph(...[
            sentence(L0, ' дозволяє звертатись до перекритих змінних, якщо записати перед їхніми іменами косу риску ', js('/'), ' (див. ', example_overlapping, '). '),
            sentence('Це працює і для перекриття вищих порядків: з кожним новим перекриттям для звернення до оригінального об\'єкту додається один символ ', js('/'), '. '),
            sentence('Таким чином, будь яке перекриття в ', L0, ' оборотне, а втратити доступ до змінної неможливо. '),
            sentence('Це особливо важливо для розв\'язання однієї з попередніх проблем: звернення до ', js(kw('return')), 'у зовнішньої функції. '),
            sentence('Але перш ніж повертатись до цього є сенс прояснити інші особливості ', L0, '. '),
        ]),
    ]),
    section('Форма запису. ', ...[
        paragraph(...[
            sentence('На цьому етапі ми вже можемо приблизно уявити з яких конструкцій буде складатись код ', L0, '. '),
            sentence('Оскільки мова не підтримує особливі форми запису для операторів, класів, циклів, галужень та присвоєнь - уся ця робота лягає на плечі функцій. '),
            sentence('Тому, по-перше, мова повинна підтримувати форму запису хоча б для їх виклику. '),
            sentence('Далі, нам потрібно вміти створювати функції які ми могли б передавати в якості аргументів або викликати безпосередньо. '),
            sentence('Тому, по-друге, потрібно підтримувати форму запису для їх оголошень. '),
            sentence('Чи можна на цьому зупинитись? '),
            sentence('Давайте спробуємо. '),
        ]),
        paragraph(...[
            sentence('За таких умов - коли підтримується лише оголошення та виклик функцій - нам не потрібні жодні службові слова. '),
            sentence('Оголошення функції ', js(f('my-function')), ' ми можемо записати просто як ', js([ f('my-function'), '() {}' ]), ', вказуючи аргументи в круглих дужках, а інструкції в квадратних. '),
            sentence('Виклик ж функції можна записати як і раніше, звичним способом: ', js([ f('my-function'), '()' ]), '. '),
        ]),
        paragraph(...[
            sentence('Якщо ми хочемо передавати аргументи або отримувати результати, то можемо записати їх через кому: ', js([ f('my-function'), '(', v('a'), ', ', v('b'), ', ', v('c'), ')' ]), '. '),
            sentence('Також я не бачу сенсу обмежуватись лише одним результатом, принаймні якщо ми можемо передавати кілька аргументів. '),
            sentence('Але, якщо ми хочемо зберегти можливість робити присвоєння через символ ', js('='), ' то доведеться замінити запис ', js([ v('a'), ' = ', f('f'), '()' ]), ' на ', js([ v('a'), ' : ', f('f'), '()' ]), ' і заборонити використовувати символ ', js(':'), ' в іменах. '),
            sentence('Тому, в загальному випадку, виклик функції буде виглядати як ', js([ v('u'), ', ', v('v'), ', ', v('w'), ' : ', f('f'), '(', v('x'), ', ', v('y'), ', ', v('z'), ')' ]), '. '),
        ]),
        paragraph(...[
            sentence('Якщо ', L0, ' не підтримує жодних інших конструкцій окрім оголошення та виклику, то ми можемо не обмежувати себе в назвах змінних так сильно, як інші мови. '),
            sentence('Наприклад, імена можуть містити пропуски, тире, а також спеціальні символи на кшталт ', js('!@#$%^&*'), '. '),
            sentence('Єдині виключення - це дужки, коми та двокрапки, оскільки вони використовуються при записі оголошення та виклику. '),
            sentence('Також нагадаємо, що стрічки та числа - це теж, по суті, просто імена змінних. '),
        ]),
    ]),
    section(sentence('Заміна ', js(kw('return')), '. '), ...[
        paragraph(...[
            sentence('Під час розмови про мовні конструкції я навмисне обійшов увагою одну дуже важливу деталь - повернення з функції. '),
            sentence('Більшість мов реалізують його як окрему конструкцію - ', js(kw('return')), ' - але ми підемо іншим шляхом. '),
            sentence('Оскільки ми прагнемо мінімізувати кількість конструкцій, то можна спробувати реалізувати повернення як частковий випадок вже наявного механізму. '),
            sentence('Вибирати особливо немає з чого, еге ж? '),
            sentence('Тому, оскільки я не знаю як реалізувати повернення з функції через оголошення іншої, доведеться обходитись викликом. '),
        ]),
        paragraph(...[
            sentence('Виходить, замість повернення з функції через спеціальну конструкцію ', js(kw('return')), ' ми натомість будемо викликати ', emp('функцію'), ' ', js([ kw('return'), '()' ]), '. '),
            sentence('Які наслідки це тягне за собою? '),
            sentence('Ну, по-перше, ця функція повинна якимось чином опинитись всередині тої функції, з якої потрібно вийти. '),
            sentence('Давайте поки вважати, що вона буде одним з аргументів, який якимось чарівним чином потрапляє всередину. '),
            sentence('Тобто наша форма запису оголошення зміниться на щось на кшталт ', js([ f('my-function'), '(', kw('return'), ', ', v('a'), ', ', v('b'), ', ', v('c'), ') {}' ]), '. '),
            sentence('Почувайтесь як вдома, пітоністи', note_self, '. '),
        ]),
        paragraph(...[
            sentence('По-друге, ми знову зіштовхуємось з нашим давнім ворогом - переповненням стеку. '),
            sentence('Тепер у нас вже два приводи боротись з ним, адже якщо виклик функції ', js(kw('return')), ' буде реалізований звичним чином, то ми неминуче зіштовхнемось з труднощами. '),
            sentence('Класична реалізація передбачає, що виклик функції (навіть без аргументів) розширює стек, а повернення - скорочує. '),
            sentence('Але якщо повернення є звичайним викликом, то у нас немає можливості скоротити стек і він приречений рости аж до переповнення. '),
            sentence('З іншого боку, дві реалізації виклику нічим не відрізняються від окремої конструкції для повернення. '),
            sentence('Можливо є сенс переглянути погляди щодо того як саме має відбуватись виклик та повернення з функції і чи повинні ці дії відрізнятись. '),
        ]),
        paragraph(...[
            sentence('Але є ще один наслідок, який дуже сильно відрізняє обраний підхід від класичного. '),
            sentence('Справа в тому, що виклик функції завжди повертає керування назад тому, хто цю функцію викликав. '),
            sentence('І якщо ', js(kw('return')), ' це звичайна функція то, як би дивно це не звучало, колись він мав би закінчити своє виконання і повернути керування назад. '),
            sentence('Між іншим, разом з керуванням можуть бути повернуті і результати ', fig('виклику'), '. '),
            sentence('І це дуже дивно, адже ми звикли вважати ', js(kw('return')), ' точкою неповернення. '),
        ]),
        paragraph(...[
            sentence('Починаючи з цього місця питань стає лише більше. '),
            sentence('Наприклад, чи можна викликати ', js(kw('return')), ' декілька разів? '),
            sentence('Якщо так, то куди саме буде відбуватись ', fig('повернення'), '? '),
            sentence('Контроль буде передано туди ж, куди й попереднього разу? '),
            sentence('Якщо так, то як це взагалі можливо? '),
            sentence('Якщо ж ні, то що повинно відбуватись після другого виклику ', js(kw('return')), ' або, наприклад, при досягненні кінця функції? '),
            sentence('Простих відповідей, на жаль, у мене немає. '),
        ]),
    ]),
    section('Передача продовжень? ', ...[
        paragraph(...[
            sentence('Описана мною ідея, як виявилось, зовсім не нова. '),
            sentence('Її називають передачею продовження і часто використовують в якості проміжного представлення для спрощення оптимізації. '),
            sentence('Також, як вже можна було здогадатись, підхід дуже нагадує звичайнісінькі функції зворотнього виклику. '),
            sentence(''),
            sentence(''),
        ]),
        paragraph(...[
