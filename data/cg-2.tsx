...
export default article([ `Програмування комп'ютерної графіки. `, `Екран, пікселі та колір. ` ], ...[
    section(`Екран. `, ...[
        paragraph(...[
            ...
            sentence(`Варто зауважити, що ті екрани з якими ми зіштовхуємось в повсякденному житті не повністю відповідають своєму представленню в сфері графіки. `),
        ...
        paragraph(...[
            ...
            sentence(`Всі пікселі на сітці екрану вирівняні, їх кількість по горизонталі, вертикалі і в сумі завжди є цілою і всі вони займають однакову площу. `),
            sentence(`В цьому плані найкращою характеристикою екрану є його роздільна здатність, яка зазвичай вимірюється в мегапікселях (мільйонах пікселів). `),
        ]),
        paragraph(...[
            ...
            sentence(`Деколи говорячи про FullHD її можуть називати 1080p, а, наприклад, QHD зі сторонами 2560 і 1440 пікселів - 1440p. `),
            sentence(`Це частково пов'язано зі стандартами зображень для телебачення, де закінчення `, i, ` та `, p, ` вказують на характер передачі даних про окремий кадр. `),
            // @todo: перевірити
            sentence(`У випадку, якщо передається лише кожен другий рядок кадру використовується закінчення `, i, ` - interleaved, а у випадку повної передачі `, p, ` - persistent. `),
            sentence(`Для віртуальних екранів зазвичай це немає жодного значення, оскільки в більшості випадків при виводі на екран передається уся інформація про кадр. `),
        ]),
