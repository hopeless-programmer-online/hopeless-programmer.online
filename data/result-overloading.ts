...
const code_2 = illustration(
    sentence(`Використання кваліфікатора `, cpp(kw(`const`)), ` може створити ілюзію перевантаження за результатом. `),
    cpp(
        [ kw(`class`), ` `, c(`X`), ` {` ],
        [ `    `, c(`int`), `   `, f(`f`), `();` ],
...
const code_3 = illustration(
    sentence(`З коду не є очевидно яку саме функцію потрібно викликати. `),
    cpp(...[
        [ c(`int`), `   `, f(`f`), `();` ],
        [ c(`float`), ` `, f(`f`), `();` ],
        [ `` ],
...
const code_4 = illustration(
    sentence(`З коду не є очевидно з котрого з просторів імен потрібно викликати функцію. `),
    cpp(...[
        [ kw(`namespace`), ` `, c(`A`), ` {` ],
        [ `    `, c(`void`), ` `, f(`f`), `();` ],
        [ `}` ],
...
const code_6 = illustration(
    sentence(`Вибір функції без результату за замовчуванням.`),
    cpp(...[
        [ c(`void`), ` `, f(`f`), `();` ],
        [ c(`int`), `  `, f(`f`), `();` ],
        [ `` ],
...
const code_7 = illustration(
    sentence(`Виклик функції, результат якої збігається з типом аргументу.`),
    cpp(...[
        [ c(`int`), `   `, f(`f`), `();` ],
        [ c(`float`), ` `, f(`f`), `();` ],
        [ `` ],
...
const using_namespace = cpp( kw(`using`), ` `, kw(`namespace`) );
...
export default article([ sentence(`Думки вголос. `), sentence(`Перевантаження за результатом. `), ], { created : new Date('2019-01-22') }, ...[
    section([], ...[
        paragraph(...[
            ...
            `Наслідком такого підходу є неможливість існування двох функцій з однаковими іменами, які відрізняються лише типом результату (див. `, code_1, `). `,
    ...
    section(`Винятки`, ...[
        paragraph(...[
            ...
            sentence(`Наприклад, в `, link(`мові програмування С++`, cppwiki), ` можливе перевантаження методу за типом результату, але лише якщо два таких методи окрім цього відрізняються ще й кваліфікатором `, cpp(kw(`const`)), ` (див. `, code_2, `). `),
    ...
    section(`Аргументація`, ...[
        paragraph(...[
            ...
            sentence(`І справді, якщо розмістити в єдиній інструкції виклик такої двозначної функції – компілятор не матиме жодних об'єктивних підстав для вибору між ними (див. `, code_3, `). `),
    ...
    section(`Контраргументація`, ...[
        ...
        paragraph(...[
            ...
            sentence(`Якщо вкладені об'єкти таких просторів розгорнути в поточну область за допомогою інструкції `, using_namespace, ` (див.`, code_4, `), то при зверненні до цих об'єктів-близнюків компілятор не зможе зорієнтуватись котрий з них нам необхідний і зупинить компіляцію. `),
            sentence(`Розробнику доведеться вирішити цю неоднозначність за допомогою оператора `, cpp(`::`), `, явно вказавши всередині якого з просторів імен необхідно шукати об'єкт. `),
        ]),
        code_4,
        paragraph(...[
            sentence(`Ще одним прикладом є використання оператора `, auto, ` в парі з, несподівано, перевантаженими функціями. `),
            sentence(`Якщо спробувати ініціалізувати змінну оголошену через `, auto, ` перевантаженою функцією, то компілятор не зможе вивести її тип (див. `, code_5, `). `),
    ...
    section(`Можливості`, ...[
        ...
        paragraph(...[
            ...
            sentence(`Було б природно робити вибір в її користь по замовчуванню, якщо мова йде про виклик функції, результат якої нікуди не передається і ніде не зберігається (див.`, code_6, `). `),
        ]),
        code_6,
        paragraph(...[
            sentence(`Автоматизацію процесу вирішення неоднозначностей також можна було б реалізувати по аналогії з виведенням в шаблонах (див. `, code_7, `). `),
