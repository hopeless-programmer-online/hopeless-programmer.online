...
const pipeline = illustration(`Будова графічного конвеєра.`,
...
const screen = illustration(`Буфер кадру з роздільною здатністю 12x8.`,
    new documenting.ImageIllustrationContent({ Url : `/media/cg-2/screen.svg` }),
);
const rasterization = illustration(`Растеризація векторних фігур.`,
...
exports = module.exports = new h.DocumentResource({
    Document : document(
        ...
        section(`Будова графічного конвеєра.`, ...[
            paragraph(...[
                sentence(`Робота графічного конвеєра зводиться до обробки та виводу об'єктів на екран. `),
                sentence(`Він розділений на окремі етапи: фіксовані та програмовані, а також взаємодіє зі сховищами даних - буферами даних, кадрів та текстурами. `),
            ]),
            pipeline,
            paragraph(...[
                sentence(`Для виводу, графічні об'єкти розбиваються на окремі графічні примітиви, з якими може працювати GPU. `),
                sentence(`Далі, ці примітиви зберігаються в його оперативній пам'яті разом з іншими даними необхідними для виводу і починається їх обробка `, pipeline, `. `),
        ...
        section(`Графічні примітиви.`, ...[
            paragraph(...[
                ...
                sentence(`Решту об'єктів, в тому числі й геометричні примітиви - сфери, прямокутники, циліндри - збирають саме з них. `),
                sentence(`Графічні примітиви складаються з вершин: одної, двох та трьох відповідно, а кожна вершина має фіксований набір атрибутів. `),
            ...
            // тут картинка з атрибутами і примітивами
            paragraph(...[
                sentence(`Ефективність конвеєра сильно залежить від того, наскільки одноманітною є його робота. `),
                sentence(`Він влаштований таким чином, що експлуатує "шаблонність" алгоритмів при обробці вершин. `),
            ...
            // тут картинка з конвеєром, який обробляє вершини
            paragraph(...[
                sentence(`Процес обробки примітивів завершується обчисленням розташування їх вершин в межах області виводу. `),
                sentence(`Можна вважати, що ця область є віртуальним екраном з координатною сіткою, на якій ми й розміщуємо вершини. `),
                sentence(`Розташування кожної з них описується четвіркою чисел: x, y, z і w, що лежать в діапазоні [-1, +1] `, viewport, ` і не залежать від співвідношення сторін екрану. `),
        ...
        section(`Буфер кадру та фрагменти.`, ...[
            paragraph(...[
                ...
                sentence(`Кожен такий фрагмент може бути заповнений суцільним кольором, а разом вони формують регулярну прямокутну сітку, яка нагадує мозаїку `, screen, `. `),
                sentence(`Така форма представлення - у вигляді прямокутної сітки фрагментів - називається растровою. `),
            ...
            paragraph(...[
                ...
                sentence(`Основною моделлю для роботи з виводом кольору на монітор є RGB або RGBA`, note(`RGBA: Red - червоний, Green - зелений, Blue - синій та Alpha - альфа.`), ` `, rgb, `, у той час як, наприклад, при друці використовується CMY`, note(`CMY: Cyan - блакитний, Magenta - пурпурний, Yellow - жовтий.`), ` `, cmy, `. `),
            ]),
            [
                rgb,
                cmy,
            ...
            paragraph(...[
                sentence(`Модель RGB передбачає, що колір виводиться на чорній поверхні за допомогою змішування трьох окремих каналів з різною інтенсивністю: червоного, зеленого та синього. `),
            ...
            paragraph(...[
                ...
                sentence(`Хоча діапазон значень [0, 1] є дуже зручним при роботі з інтенсивністю кольору, але важливо розуміти що фрагменти буфера кадру можуть використовуватись і для інших цілий. `),
                sentence(`Тому, деколи такі "кольори" можуть виходити далеко за межі одиниці і навіть ставати від'ємними. `),
            ]),
        ]),
        section(`Растеризація.`, ...[
            paragraph(...[
                sentence(`Отож, одна сторона графічного конвеєра обробляє векторні примітиви, у той час як інша - растрові пікселі. `),
                sentence(`Тому, для перетворення векторного "зображення" в растрове на певному етапі конвеєра відбувається спеціальний процес - растеризація `, rasterization, `. `),
                sentence(`Під час нього, серед усіх фрагментів буфера кадру визначаються ті, які належать відповідному графічному примітиву. `),
                sentence(`Далі, кожен з цих фрагментів буде оброблятись і тестуватись на подальших етапах, аж доки не буде визначено його кінцевий колір. `),
            ]),
            rasterization,
            paragraph(...[
                sentence(`Після растеризації, фрагменти проходять ще декілька етапів обробки, аж доки не відбудеться їх кінцеве змішування. `),
                sentence(`Під час нього їх колір буде змішаний з кольорами фрагментів, які вже перебувають у буфері кадру. `),
            ]),
        ]),
        section(`Підсумки`, ...[
            paragraph(...[
                sentence(`Отож, на поверхневому рівні, робота графічного конвеєра зводиться до виводу на екран графічних примітивів. `),
                sentence(`Примітиви, задані у векторному представленні, обробляються та растеризуються. `),
                sentence(`Після цього відбувається обробка фрагментів отриманого растрового зображення, які потім змішують з картинкою на екрані. `),
            ]),
            paragraph(...[
                sentence(`Кожен окремий етап графічного конвеєра наповнений низкою деталей, тому їх варто розглядати окремо. `),
                sentence(`Ця ж стаття дає лише розуміння загальної структури конвеєра та його функції. `),
                sentence(`Далі ми розглянемо кожен етап більш детально. `),
                sentence(`Зокрема, поговоримо про програмовані етапи обробки вершин, фрагментів, контурів, геометрії, розбиття, а також про низку дрібних фіксованих етапів, таких як змішування, тести глибини, відбраковування та інші. `),
            ]),
        ]),
    ),
});
