...
exports = module.exports = new h.DocumentResource({
    Document : document([ `Як впихнути Bullet Physics. `, `В браузер.`, `Через TypeScript. `, `З використанням npm.` ], {},
        ...
        section(`Інтеграція з Heroku.`, ...[
            paragraph(...[
                sentence(`Для того аби поділитись нашою невеликою демонстраційною сценою зі світом ми використаємо безкоштовний хостинг Heroku, який вміє працювати з проектами на Node.js. `),
                sentence(`Для початку потрібно зареєструватись на https://www.heroku.com і підтвердити свій обліковий запис через поштову скриньку. `),
                sentence(`Далі, потрібно додати новий проект та з'єднати його з репозиторієм на github. `),
                sentence(`Після цього можливо буде увімкнути автоматичне розгортання проекту, після чого Heroku буде повторно збирати та розгортати ваш проект при кожному коміті. `),
            ]),
            paragraph(...[
                sentence(`Остання важлива деталь - це порт для прослуховування. `),
                sentence(`В Heroku встановлює його сам, а дізнатись його в Node.js можна за допомогою змінної оточення process.env.PORT. `),
                sentence(`Оскільки під час розробки ми не плануємо визначати цю змінну - є сенс прослуховувати якийсь власний порт (наприклад, 8080) якщо вона не встановлена. `),
            ]),
            illustration(``, js(...[
                [ `import path from "path";` ],
                [ `import express from "express";` ],
            ...
            paragraph(...[
                sentence(`Ось це й усе! `),
                sentence(`Тепер у нашому розпорядженні є повністю функціональна демонстраційна сцена з фізикою та графікою, що автоматично розгортається на платформі Heroku. `),
            ]),
        ]),
    ),
});
